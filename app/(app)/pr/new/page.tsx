...
export default async function NewPRPage() {
  ...
  const [uoms, mgs, plants, pgs, ccs, gls, crs, assets, vendors] = await Promise.all([
    ...
    supabase.from("vendors").select("id, vendor_code, legal_name, payment_terms, currency").eq("status", "active").order("legal_name")
  ]);

  return <NewPRClient masters={{
    uoms: uoms.data || [], mgs: mgs.data || [], plants: plants.data || [],
    pgs: pgs.data || [], ccs: ccs.data || [], gls: gls.data || [],
    crs: crs.data || [], assets: assets.data || [], vendors: vendors.data || []
  }} />;
}
